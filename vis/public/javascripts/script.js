...
/*        config.constraints.forEach(function(c) {
    ...
    for (var name in graph.data) {
        ...
        for (var depIndex in obj.depends) {
            var link = {
                target : graph.data[obj.depends[depIndex]],
                source : obj
            };
...
//        .charge(config.graph.charge)
  ...
  $.get('/stylesheets/svg.css', function(inlineCSS) {
    ...
    graph.svg.append('style').text(inlineCSS);
...
function tick(e) {
    ...
    for (var name in graph.data) {
      ...
      obj.x = Math.max(20, obj.x);
      obj.y = Math.max(20, obj.y);
      obj.x = Math.min(graph.width - 20, obj.x);
      obj.y = Math.min(graph.height - 20, obj.y);
    }
